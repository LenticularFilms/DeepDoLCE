...
def create_overlap(x,y, quantile_x=(0.,0.99), cmap='magma',max_alpha=0.5):
    ...
    #img = Image.fromarray((255*y.squeeze().transpose(1,2,0)).numpy().astype('uint8').squeeze(), 'RGB')

    if x.shape[1] == 3:
        img_1 = Image.fromarray(np.uint8((x_plot.transpose(1,2,0)) * 255)).convert("RGBA")
    else:
...
def add_tensorboard_images(writer, sample, output, global_step,args=None):
    ...
    for i in range(0,max_images):
        ...
        if sample['x'].shape[1] == 3:
            img_x.append(torch.from_numpy(np.asarray(create_RGBimage(sample['x'][i].cpu().detach().clone().numpy(),
                                    quantile_x=(0.,1.)))).permute(2,0,1).clone())
            img_y_pred.append(torch.from_numpy(np.asarray(create_RGBimage(output['y_pred'][i].cpu().detach().clone().numpy(),
                                    quantile_x=(0.,1.)))).permute(2,0,1).clone())
            img_y.append(torch.from_numpy(np.asarray(create_RGBimage(sample['y'][i].cpu().detach().clone().numpy(),
                                    quantile_x=(0.,1.)))).permute(2,0,1).clone())
        else:
            #img_x.append(torch.from_numpy(np.asarray(create_RGBimage(sample['x'][i].cpu().detach().clone().numpy(),
            #                        quantile_x=(0.,1.)))).permute(2,0,1).clone())
            ...
            img_y_pred.append(torch.from_numpy(np.asarray(create_overlap(sample['x'][i].cpu().detach().clone().numpy(),
                                    output['y_pred'][i].cpu().detach().clone().numpy(),
                                    quantile_x=(0.,1.),cmap='viridis',max_alpha=0.5))).permute(2,0,1))

    if len(img_x) > 0:
        img_x = torchvision.utils.make_grid(img_x,nrow=max_images) #,normalize=True)
        writer.add_image("input",img_x,global_step=global_step)
    ...
    writer.add_image("groundtruth",img_y,global_step=global_step)
