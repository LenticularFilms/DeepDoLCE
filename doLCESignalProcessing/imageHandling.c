...
int new_rgbImage( rgbImage_t *rgbImage, int width, int height ){
    ...
    if ( rgbImage->img!=NULL || rgbImage->memState!=0 ) {
        status = -1;
        printf( "ERROR: new_rgbImage(): Invalid argument 'rgbImage->img!=NULL || rgbImage->memState!=0'.\n" );
    } else if ( width<1 || height<1 ) {        
        status = -1;
...
/////////////////// NEW METHOD TO GET LINE POSITIONS BY EXTRAPOLATING AND COMPARING POSITIONS FOR THE RASTER (=positions of Lenses)
int generate_lensPosProfile( glProfile_t *glProfile, float subPixelPrecisionRasterSpacing, int *nPeaks ) {
  int status = 0;
  
  int threshold = 0;
  
  int i_prev, i_next, i, ii, i_hook_left, i_hook_right;
  int i_start, i_end;
  int diff;
  int dist = 0;
  float fixDist;
  int nPeaksToFix;
  int nRecoveredPeaks_left, nRecoveredPeaks_right;
  ...
  float lensWidth = subPixelPrecisionRasterSpacing;
  int rasterSpacing = (int)(lensWidth + 0.5);

  printf( "////////////////////////////////////////// generate line positions by extrapolation and comparison\n" );
  printf ( "//////////////////////////////////////// lensWidth = %f\n", lensWidth );
  ...
  if ( glProfile->glSum==NULL || glProfile->memState!=1 ) {
    ...
    status = -1;
    printf( "ERROR: Invalid argument 'lensWidth=%f < 7'/n", lensWidth  );
  }
  ...
  if ( status==0  && *nPeaks>0 ) {
    if ( (regPeaks = malloc( glProfile->length*sizeof(int) )) == NULL ){
      status = -1;
      printf( "ERROR: generate_lensPosProfile(): Failed to allocate memory for 'regPeaks'.\n" );                
    }
    else {
      memState_regPeaks = 1;
      for ( i=0; i<glProfile->length; i++ ) {
  ...
  if ( status==0 && *nPeaks>0 ) {
    printf( "> looking for regular peaks, i.e. such spaced by lensWidth=%f, starting from first peak detected at %d\n", lensWidth, i_prev );
    extractedRegDistances = 0;
    i = i_prev + 1;
    while ( i<glProfile->length ) {
      if ( glProfile->glSum[i] > threshold ) {
        diff = (int)(i_prev + lensWidth - i);
	printf( "i_prev = %d, lensWidth = %f, i = %d, diff = %d, or = %f\n", i_prev, lensWidth, i, diff, i_prev+lensWidth-i ); 
        if ( diff == 0 ) { //////////////////////////////////////////// modulus of difference smaller than one
  ...
  //printf( "++++++++++++++++++++++++++++++++++++++++threshold = %d\n", threshold );
  // now find the good minima from glHist by extrapolating from regPeaks and comparing to glHist
  // a) left end: find first regPeak then search for good minima in glHist
  if ( status==0 && *nPeaks>0 ) {
    i_hook_left = -1;
    diff = -1;
    for ( i=0; i<glProfile->length; i++ ) {
      if ( regPeaks[i] == 1 && i_hook_left == -1 ) {
	i_hook_left = i;
	nRecoveredPeaks_left = 0;
	printf( "--------------> found first sure Peak (=black line) at i_hook_left = %d\n", i_hook_left );
	for ( ii=i_hook_left; ii>0; ii-- ) {
	  if ( glProfile->glSum[ii] > threshold ) {
	    //printf( "checking glProfile->glSum[%d] = %d\n", ii, glProfile->glSum[ii] );
	    if ( (int)(i_hook_left - ii - (nRecoveredPeaks_left+1)*lensWidth ) == 0 ) {
	      regPeaks[ii] = 2;
	      nRecoveredPeaks_left++;
	    }
	  }
	}
      }
    }
    printf( "************** recovered %d peaks at left side\n", nRecoveredPeaks_left );
    /// clean out the interstials on this side
    for ( i=0; i<i_hook_left; i++ ) {
      if ( glProfile->glSum[i] > 0 && regPeaks[i] == 0 ) {
	glProfile->glSum[i] = 0;
	printf( "Cleaned out interstitial at i=%d\n", i );
      }
    }
  }
  // b) right end: find last regPeak then search for good minima in glHist
  if ( status==0 && *nPeaks>0 ) {
    i_hook_right = -1;
    diff = -1;
    for ( i=glProfile->length-1; i>0; i-- ) {
      if ( regPeaks[i] == 1 && i_hook_right == -1) {
	i_hook_right = i;
	nRecoveredPeaks_right = 0;
	printf( "----------------------> found last sure Peak (=black line) at i_hook_right = %d\n", i_hook_right );
	for ( ii=i_hook_right; ii<glProfile->length; ii++ ) {
	  if ( glProfile->glSum[ii] > threshold ) {
	    //printf( "checking glProfile->glSum[%d] = %d\n", ii, glProfile->glSum[ii] );
	    if ( (int)(ii - i_hook_right - (nRecoveredPeaks_right+1)*lensWidth ) == 0 ) {
	      regPeaks[ii] = 2;
	      nRecoveredPeaks_right++;
	    }
	  }
	}
      }
    }
    printf( "************** recovered %d peaks on the right side\n", nRecoveredPeaks_right );
    /// clean out the interstials on this side
    for ( i=i_hook_right; i<glProfile->length; i++ ) {
      if ( glProfile->glSum[i] > 0 && regPeaks[i] == 0 ) {
	glProfile->glSum[i] = 0;
	printf( "Cleaned out interstitial at i=%d\n", i );
      }
  ...
  if ( status==0 ) {
    i_prev = i_hook_left;
    for ( i=i_hook_left+1; i<i_hook_right+1; i++ ) {
      if ( regPeaks[i] == 1 ) {
	diff = (int)(i - i_prev - lensWidth);
	if ( diff == 0 ) {
	  i_prev = i;
	}
	else if ( diff > 0 ) {
	  nLenses = (int)((i-i_prev)/lensWidth+0.5);
	  printf( "-------------------------------> found a gap of nLenses = %d\n", nLenses );
	  for ( ii=1; ii<nLenses; ii++ ) {
	    i_next = (int)(ii*lensWidth+0.5);
	    if ( glProfile->glSum[i_next] > threshold ) {
	      regPeaks[i_next] = 2;
	    } /////////////////////////////// potentially also should look one pixel before and after i_next
	  }
	  i_prev = i;
	}
	else {
	  status = -1;
	  printf ( "Problem with regular space Peaks!!!!!!!!!!!!!\n" );
	}
      }
    }
  }
...
/////////////////////////////////////////////////////////////
/// new attempt to improve regularization of raster /////////
////////////////////////////////////////////////////////////



int regularize_peakRaster_mod( glProfile_t *glProfile, float lensWidth, int *nPeaks ) {
  int status = 0;
  ...
  int i_hook_left, i_hook_right;
  int nRecoveredPeaks_left, nRecoveredPeaks_right, nRecoveredPeaks_center;
  int diff;
  float dev;
  ...
  int rasterSpacing = (int)(lensWidth + 0.5);
  printf( "rasterSpacing from lensWidth: %d\n", rasterSpacing );
  ...
  if ( status == 0 ) {
    /// for devel: print all entries other than 0
    /*
    count = 0;
    printf( "gProfile(i, value):\n" );
    for ( i=0; i<glProfile->length; i++ ) {
      if ( glProfile->glSum[i] != 0 ) {
	count++;
	printf( "(%d, %d)\t", i, glProfile->glSum[i] );
      }
    }
    printf( "\nTotal of %d peaks\n", count );
    */    
    ...
    if ( i_prev == -1 ) { 
      ...
      *nPeaks = 1;
      //printf( ">>>>>>>>>>>>>>>>>>>>>>> First potential raster point found at i = %d\n", i_prev );
    }
  }
  if ( status==0  && *nPeaks>0 ) {
    if ( (regPeaks = malloc( glProfile->length*sizeof(int) )) == NULL ){
  ...
  if ( status==0 && *nPeaks>0 ) {
    ...
    i = i_prev + 1;
    count = 1;
    printf ( "distances between peaks:\n" );
    while ( i<glProfile->length ) {
      if ( glProfile->glSum[i] > threshold ) {
        dist = i - i_prev;
	count++;
	printf( "%d->%d:%d ", i_prev, i, dist );
        //if ( dist == rasterSpacing ) { ///// probably to hard a criterion ! ---> changed it to use lensWidth
	if ( abs((int)(dist - lensWidth)) < 2 ) { /// apparently the downstream processing suppressed the influence of this part, either < 1 or 2 is fine
	  regPeaks[i_prev] = regPeaks[i] = 1;
          extractedRegDistances++;
	  printf( "reg    " );
        }
    ...
    printf( "Detected %d regular spaces between %d local minima of glProfile\n", extractedRegDistances, count );
    *nPeaks = count; // This is a bit dangerous because the irregular spaced peaks are counted but not in the list of regPeaks yet
    count = 0;
    //printf( "regPeaks(i, value):\n" );
    for ( i=0; i<glProfile->length; i++ ) {
      if ( regPeaks[i] == 1 ) {
	count++;
	//printf( "(%d, %d)\t", i, regPeaks[i] );
      }
      else if ( regPeaks[i] != 0 ) {
	printf( "WARNING: Strange peak detected at %d, value &d\n", i, regPeaks[i] );
      }
    }
    printf( "This corresponds to a total of %d peaks with value 1 (= regular peaks extracted directly from local minima)\n", count );
    nRegPeaks = count;
  }


  ///////////////// NEW APPROACH TO DEAL WITH REMAINING *nPeaks - nRegPeaks: GO FIND THEM AND CHECK FOR NEXT regPeaks, DECIDE WHETHER TO KEEP OR NOT //////////////////////
  ///////////// FIRST WITH A rather hard approach to avoid having several next to each other or interstitials
  if ( status==0 && *nPeaks>0 ) {
    printf ( "Checking the remaining %d peaks and deciding if they fit the raster or not (distance to neighbors is an integer multiple of lensWidth)", *nPeaks-nRegPeaks );
    

  
  }
  /// if the above works skip the section below!!!!!!!!!!!!!!!!!!!!!!!!!!
    ///////////////////////////////////////////////////////////////////////////////////////////
    // now find more good minima from glProfile by extrapolating from already detected regPeaks and comparing to the local minima reported in glHist
  // a) left end: find first regPeak then search for good minima in glHist
  if ( status==0 && *nPeaks>0 ) {
    i_hook_left = -1;
    //diff = -1;
	...
	i_hook_left = i;
	nRecoveredPeaks_left = 0;
	printf( "--------------> found first sure peak (=green line in debug image) at i_hook_left = %d\n", i_hook_left );
	for ( ii=i_hook_left-1; ii>0; ii-- ) {
	  if ( glProfile->glSum[ii] > threshold ) { ////////////////////////////////////////// DANGER: IF SEVERAL TIGHT SPACED LOCAL MINIMA EXITS AROUND A GOOD LOCATION THEY WILL GET INCORPORATED
	    printf( "--- checking glProfile->glSum[%d] = %d\n", ii, glProfile->glSum[ii] );
	    if ( abs((int)(i_hook_left - ii - (nRecoveredPeaks_left+1)*lensWidth)) < 2 ) {
	      regPeaks[ii] = 2;
	      nRecoveredPeaks_left++;
	    }
	  }
  ...
  if ( status==0 && *nPeaks>0 ) {
	...
	i_hook_right = i;
	nRecoveredPeaks_right = 0;
	printf( "----------------------> found last sure Peak (= green line in debug image) at i_hook_right = %d\n", i_hook_right );
	for ( ii=i_hook_right+1; ii<glProfile->length; ii++ ) {
	  if ( glProfile->glSum[ii] > threshold ) { ////////////////////////////////////////// DANGER: IF SEVERAL TIGHT SPACED LOCAL MINIMA EXITS AROUND A GOOD LOCATION THEY WILL GET INCORPORATED
	    printf( "--- checking glProfile->glSum[%d] = %d\n", ii, glProfile->glSum[ii] );
	    if ( abs((int)(ii - i_hook_right - (nRecoveredPeaks_right+1)*lensWidth) ) < 2 ) { /////// THIS LOOKS WEIRD
	      regPeaks[ii] = 2;
	      nRecoveredPeaks_right++;
	    }
	  }
  ...
  // c) center region
  if ( status==0 ) {
    //printf( ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> checking in the center....\n" );
    i_prev = i_hook_left;
    nRecoveredPeaks_center = 0;
    for ( i=i_hook_left+1; i<i_hook_right+1; i++ ) {
      if ( regPeaks[i] == 1 ) {
	dist = i - i_prev;
	//printf( "DIST = %d, LENSWIDTH = %f\n", dist, lensWidth );
	if ( abs(dist -lensWidth) < 2 ) { //////////////// USED fabssf() ?????
	  i_prev = i;
	  //printf( "reg dist: %d --> %d, abs(dist-lensWidth)=%d\t", i_prev, i, abs(dist -lensWidth) );
	}
	else if ( abs( dist - lensWidth*(int)(dist/lensWidth+0.5) ) < 2 ) { // accept a window of fixed size for raster points further than next expected 
	  nLenses = (int)((i-i_prev)/lensWidth+0.5);
	  printf( "\n-------------------------------> found a gap: %d -> %d, dist=%d => nLenses = %d\n", i_prev, i, dist, nLenses );
	  for ( ii=1; ii<nLenses; ii++ ) {
	    i_next = (int)(ii*lensWidth+0.5);
	    if ( glProfile->glSum[i_next] > threshold ) {
	      regPeaks[i_next] = 2;
	      printf ( "****************** recovered a peak at %d\n", i_next );
	      nRecoveredPeaks_center++;
	    } /////////////////////////////// potentially also should look one pixel before and after i_next, YES DO THAT LATER
	  }
	  i_prev = i;
	}
	else {
	  status = -1;
	  printf ( "Irregular or large gap: i = %d, i_prev = %d, dist = %d, lensWidth = %f, abs( dist - lensWidth*(int)(dist/lensWidth+0.5) = %d\n", i, i_prev, dist, lensWidth, abs( dist - lensWidth*(int)(dist/lensWidth+0.5)) );
	}
      }
    }
    printf( "************** recovered %d peaks in the center part\n", nRecoveredPeaks_center );
    // clean out potential interstitials also here
    for ( i=i_hook_left+1; i<i_hook_right; i++ ) {
      if ( glProfile->glSum[i] > 0 && regPeaks[i] == 0 ) {
	glProfile->glSum[i] = 0;
	printf( "Cleaned out interstitial at i=%d\n", i );
      }
  ...
  //////////////////////////////////////////////////// HERE TOO MANY EXTRACTED PEAKS GET KILLED //////////////////// CURRENTLY JUST OMITTING THIS PART //////////////////////////////
  /*
  if ( status==0 && *nPeaks>0 ) {
    i_prev = -1;
    dist = -1;
    ...
    for ( i=0; i<glProfile->length; i++ ) {
      ...
      if ( regPeaks[i] > 0 ) {
        if ( i_prev == -1 ) {
          ...
          dist = i - i_prev;
	  dev = fabsf( dist - lensWidth*(int)(dist/lensWidth));
          //if ( dist !=rasterSpacing && (dist<(3*rasterSpacing)/2) ) { //this is the old version
	  printf ( "dist = %d, dev = fabsf( dist - lensWidth*(int)(dist/lensWidth)) = %f\n", dist, dev );	  
	  if ( dev > 1.5 ) {  ////////////////////////////////////////////////////////////////////////////////// WORK ON THIS with fabsf and < 1.5 o similar WELL should be > 1.5, right
	  //if ( ((int)(dist-lensWidth) != 0) && (dist<(3*lensWidth/2)) ) {
            //printf( "> removing peaks at %d and %d spaced by dist=%d!=rasterSpacing && dist<(3*rasterSpacing)/2\n",i_prev, i, dist );
            regPeaks[i_prev] = regPeaks[i] = -1;
            nRemovedPeaks += 2;
          }
          i_prev = i; /// IDEA: if previous was good only kill one and set i_prev to -1 to find the next peak and start the testing over
        }
        printf( "%d\t%d\t%d\n", i, regPeaks[i], dist );
        nRegPeaks++;
      }
  ...
  */
  ...
  /// try going on without the peak killer part above, WATCGH OUT THE if STATEMENTS TESTING FOR nRegPekas, etc. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  printf( "nRegPeaks = %d, nPeaks =%d , count = %d\n", nRegPeaks, *nPeaks, count );
  
  
  if ( status==0 ) { //&& nRegPeaks>0 ) {
    printf( "> Recovering overlooked peaks and filling in missing peaks to form a complete regular raster\n" );
    // left boundary region first
    for ( i=0; i<glProfile->length; i++ ) {
      if ( regPeaks[i] > 0 ) {
        i_prev = i;
    ...
    printf( "leftmost extracted /recovered regular peak is at i=%d\n", i_prev );
    nAddedPeaks_left = 0;
    nLenses = (int)(i_prev/lensWidth+0.5);
    printf( "Need to extrapolate positions of nLenses = %d peaks\n", nLenses );
    for ( ii=1; ii<nLenses; ii++ ) {
      i = (int)(i_prev - lensWidth*ii +0.5);
      regPeaks[i] = 2;
      nRegPeaks++; // THAT'S NEEDED CONSISTENLY FURTHER UP AS WELL !!!!!!! use *nPeaks
      nAddedPeaks_left++;
    }
    printf( "Added %d peaks at left boundary by new method.\n", nAddedPeaks_left );
    
    // right boundary region
    for ( i=glProfile->length-1; i>i_prev; i-- ) {
      if ( regPeaks[i] > 0 ) {
        i_next = i;
        i = i_prev;
      }
    }
    printf( "rightmost extracted /recovered regular peak is at i=%d\n", i_next );
    nAddedPeaks_right = 0;
    nLenses = (int)((glProfile->length - i_next)/nLenses);
    printf( "Need to extrapolate positions of nLenses = %d peaks\n", nLenses );
    for ( ii=0; ii<nLenses; ii++ ) {
      i = (int)(i_next + lensWidth*ii + 0.5);
      regPeaks[i] = 2;
      nRegPeaks++;
      nAddedPeaks_right++;
    }
    printf( "Added %d peaks at right boundary by new method.\n", nAddedPeaks_right );
    /// for devel: print all entries other than 0
    //
    count = 0;
    printf( "****************************************************************\n" );
    ...
    //
    // and the middle region with a slightly tuned way of filling in missing peaks
    i_start = i_prev;
    i_end = i_next;
    printf( "now looking for too long spaces between %d and %d\n", i_start, i_end );
    nAddedPeaks_middle = 0;
    for ( i=i_start+1; i<i_end; i++ ) {
      if ( regPeaks[i] < 1 ) {
        ...
        i_next = ii;
        dist = (i_next-i_prev);
        printf( "\tSpace of %d pixels (from %d to %d)\t", dist, i_prev, i_next );
        nPeaksToFix = (int)(dist/lensWidth + 0.5) - 1;
	if ( nPeaksToFix == 0 ) {
	  printf( "OK!\n" );
	  i_prev = i_next;
	  i = i_next + 1;
	  ...
	  //fixDist = (float)dist/(nPeaksToFix+1);
	  printf( "--> npeaksToFix = %d, spaced by %f : ", nPeaksToFix, lensWidth );   
	  for ( ii=1; ii<=nPeaksToFix; ii++ ) {
	    printf( "|%d ", i_prev+(int)(ii*lensWidth + 0.5) );
	    regPeaks[i_prev+(int)(ii*lensWidth + 0.5)] = 3;
	    nAddedPeaks_middle++;
	  }
	  printf( "\n" );
	  i_prev = i_next;
	  i = i_next + 1;
        ...
        printf( "peak at %d present - skipping to next position -----------------------------------------------------------------888888888888888888888888-----------\n", i );
        i_prev = i;
      }
    }
    printf( "Added %d peaks in the middle to fill gaps.\n", nAddedPeaks_middle );
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////// attempting to make an all new relaxation algorithm ///////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////

int relax_regPeakRaster_mod( glProfile_t *glProfile, glProfile_t *regPeakProfile, int maxDist ) {
  int status = 0;
  ...
  if ( status == 0 ) {
    ///// first create a copy of the glProfile then smooth it glProfile_t *smoothedGlProfile,
    /*
if ( horProfile.memState == 0 ) {
    ...
    */
    ...
    /*
      if ( relaxRaster == 1 ) {
      status = copy_glProfile( &horProfile, &horProfileCpy ); // using the smoothed copy for the raster relaxation: seems not to be a good idea judged by testfilm_01.tif
      if ( status == 0 ) {
    ...
    */
	...
	// strategy: first mark all found peaks by changing the value to +=5 --> extracted: 1 --> 6, filled in: 2 --> 7
	//regPeakProfile->glSum[i] += 5;

	// create a mask to be used to locate the center of the well
	//int *mask
	...
	//printf( "i=%d, glBuf=%d, >", i, glBuf ); 
        shift = 0;
        for ( dist=-maxDist; dist<=maxDist; dist++ ) {
	  //printf( "dist=%d, gl=%d >", dist, glProfile->glSum[i+dist] );
	...
	//printf( "\n" );
        if ( shift != 0 ) {
          ...
          printf( "> peak at x-position %d was shifted %d pixels, value %d --> %d\n", i, shift, glProfile->glSum[i], glProfile->glSum[i+shift] );
	  i+=maxDist; ////////////////////////////// need to prevent shifting same peak twice, it will fall into an ill spaced side minimum!!!!!!!!!!!!! Actually this now makes it perform worse !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        }
        */
...
int relax_regPeakRaster( glProfile_t *glProfile, glProfile_t *regPeakProfile, int maxDist ) {
  int status = 0;
...
int reconstructInterpolate_colorFrame( glImage_t *glScan, positionList_t *rasterPos, rgbImage_t *rgbFrame, int *rawMode ) {
  int status = 0;
  ...
  float buf;
  int s;
  int enveloppe[1] = {1};//[3] = {1,3,1}; //[5] = {1,3,4,3,1};//[7] = {1,2,7,8,7,2,1};
  int enveloppeLength = 1;
  int enveloppeCenter = 0;
  int xBinSize, yBinSize;
  int rasterOffset;
  ...
  if ( status == 0 ) {
	...
	for ( yCol=0; yCol<rgbFrame->height; yCol++ ) {
	  ...
	  for ( channel=0; channel<3; channel++ ) {
	    rgbFrame->img[channel][yCol][rgbFrameWidth-xCol] = (int)(gls[channel]);
	      //rgbFrame->img[channel][yCol][xCol] = (int)(col[channel]);
	  }
	}
      }
...
int reconstructRAW_colorFrame( glImage_t *glScan, positionList_t *rasterPos, rgbImage_t *rgbFrame, int *rawMode ) {
  int status = 0;
  ...
  printf( "COLOR RECONSTRUCTION:   rawMode = %d\n", *rawMode );
...
/////////////// NEW: reconstructFULL_colorFrame


int reconstructFULL_colorFrame( glImage_t *glScan, positionList_t *rasterPos, rgbImage_t *rgbFrame ) {
    int status = 0;
    
    int x, y, xx, yy; // coordinates in rgb img and gl image respectively
    int channel;
    int i, j, ii;
    int yBinSize;
    ...
    int lensWidth;
    int lensNo;
    int rgbFrameWidth = rgbFrame->width - 1;
    int colVal;
    int lensStart, lensEnd;
    float lensCenter;
    int firstLens;
    float colChWidth;
    int glValue;
    ...
    else if ( rgbFrame->width != glScan->width ) {
        status = -1;
        printf( "ERROR: reconstruct_colorFrame(): Incompatible arguments 'rgbFrame->width!=glScan->width'\n" );
	} 
    
    
    // determine how many lines are to be summed up to create one line in the output rgbFrame
    if ( status == 0 ) {
      yBinSize = glScan->height/rgbFrame->height;
      printf( "yBinSize = %d\n", yBinSize );
      //printf( "Skipping mirroring currently --> should be done in post!!!!!!!!!\n" );
      
    }
    if ( status == 0 ) {
      // setting all values in the image to 0
      printf( "rgbFrame: width=%d, height=%d\n--> setting all entries to 0.\n", rgbFrame->width, rgbFrame->height );
      for ( channel=0; channel<3; channel++ ) {
	for ( y=0; y<rgbFrame->height; y++ ) {
	  for ( x=0; x<rgbFrame->width; x++ ) {
	    rgbFrame->img[channel][y][x] = 0;
	    /*
	    glValue = glScan->img[yBinSize*y][x]; //0
	    for ( j=1; j<yBinSize; j++ ) {
	      glValue += glScan->img[yBinSize*y+j][x];
	    }
	    rgbFrame->img[channel][y][x] = glValue/yBinSize;
	    */
	  }
	}
      }
      ...
      // run through each lens starting with the leftmost and fill in the color values starting at the right end of the rgb image
      //printf( "Output is mirrored to compensate for the backside (= emulsion side) scanning that is needed to see the lens raster.\n" );
      firstLens = rasterPos->list[0];
      printf( "First lens starts at %d\n", firstLens );
      // get to the position of a lens
      for ( lensNo = 0; lensNo<rasterPos->length-1; lensNo++ ) {
	lensStart = rasterPos->list[lensNo];
	lensEnd = rasterPos->list[lensNo+1];
	lensWidth = lensEnd - lensStart;
	lensCenter = (lensStart + lensEnd)/2.0;	
	colChWidth = lensWidth/3.0;
	shift = -1.0;
	range = (int)(lensWidth/7);
	chCenter[0] = (int)( lensCenter - colChWidth + shift );
	chCenter[1] = (int)( lensCenter + shift );
	chCenter[2] = (int)( lensCenter + colChWidth + shift );
	
	
	printf( "lensNo=%d, lensStart=%d, lensWidth=%d,  chCenter0=%d, chCenter1=%d, chCenter2=%d\n", lensNo, lensStart, lensWidth, chCenter[0], chCenter[1], chCenter[2] );
	// reconstruct the color information

	// just summing around the center and copying over the whole lens width
	for ( channel=0; channel<3; channel++ ) {
	  for ( y = 0; y < rgbFrame->height; y++ ) {
	    glValue = 0;
	    yy = y*yBinSize;
	    for ( j=0; j<yBinSize; j++ ) {
	      yy += j;
	      for ( ii=-range; ii<range; ii++ ) {
		glValue += glScan->img[yy][chCenter[channel]+ii];
	      }
	    }
	    glValue = glValue/yBinSize;
	    for ( i=3; i<lensWidth-3; i++ ) {
	      rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] = glValue;
	    }
	    rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+0)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-1)] = glValue*5/6;
	    rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+1)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-2)] = glValue*11/12;
	    rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+2)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-3)] = glValue*39/40;
	    //rgbFrame->img[channel][y][chCenter[(channel+1)%3]] = 0;
	    //rgbFrame->img[channel][y][chCenter[(channel+2)%3]] = 0;
	  }
	}
	
	/*
	xx = lensStart;
	for ( i=0; i<lensWidth; i++ ) {
	  xx += i;
	  //printf( "%d ", xx );
	  ...
	  for ( y = 0; y < rgbFrame->height; y++ ) {
	    ...
	    for ( j=0; j<yBinSize; j++ ) {
	      yy += j;
	      glValue += glScan->img[yy][xx];
	    }
	    glValue = glValue/yBinSize;
	    // HERE the color reconstruction is done
	    
	    //channel = (int)(i/colChWidth);
	    //x = lensStart + 3*(i%(int)colChWidth);
	    //for ( ii=0; ii<3; ii++ ) {
	    //  rgbFrame->img[channel][y][x+ii] = glScan->img[yy][xx];
	    // }
	    //for (channel = 0; channel < 3; channel++ ) { 
	    //rgbFrame->img[channel][y][xx] = glScan->img[yy][xx];
	    //}
	  }
	}
	//printf( "\n" );
	*/
	...
	for ( lensNo=0; lensNo<rasterPos->length-1; lensNo++ ) {
	  ...
	  lensEnd = rasterPos->list[lensNo+1];
	  lensWidth = lensEnd - lensStart;
	  // run through the lens and get the colors
	  for ( i=0; i<lensWidth; i++ ) {
	    xx = lensStart + i;
	    // summing the gl values over yBinSize lines
	    colVal = 0;
	    for ( yy=yBinSize*y; yy<yBinSize*(y+1); yy++ ) {
	      colVal += glScan->img[yy][xx];
	    }
	    // determine the channel to store this color value
	    channel = 0;//(i/lensWidth)*3;
	    // determine the positions x this shall be stored to (strech color channel over whole lens)
	    x = xx - firstLens; // just to try skip the streching thing for the moment -----> TESTING WAY!!!!!!
	    // x = lensStart + (i%(lensWidth/3))*4 ///// idea: strecht the position relative to colorchannel over the lens
	    rgbFrame->img[channel][y][rgbFrameWidth-x] = 125;//colVal;
...
//////// new attempt for UHR mode, i.e. including higher horizontal resolution

int reconstructUHR_colorFrame( glImage_t *glScan, positionList_t *rasterPos, rgbImage_t *rgbFrame ) {
    int status = 0;
    ...
    float factors[5] = { 0.3, 0.5, 0.7, 0.2, 0.2 }; // color channel centers r,g,b, width of a color channel and half width of lens end region: all relative to the lensWidth
    printf( "Lens division factors: r, g, b, colChWidth, lensEndHalfWidth = %.5f, %.5f, %.5f, %.5f, %.5f\n", factors[0], factors[1], factors[2], factors[3], factors[4] );
    //float function[50];
    //int funcLen = 50;
    //int buffer[50];
    int cchw;
    int lensEndRange;
    float lensEndWeights[5] = { 0.76, 0.82, 0.91, 0.945, 0.97 };
    
    //printf( "lensEndWeights[5] = { %f, %f, %f }\n", lensEndWeights[0], lensEndWeights[1], lensEndWeights[2] );
	...
	lensEnd = rasterPos->list[lensNo+1];
	lensWidth = lensEnd - lensStart;	
	chCenter[0] = (int)( lensWidth * factors[0] ); //Center - colChWidth + shift );
	chCenter[1] = (int)( lensWidth * factors[1] ); //Center + shift );
	chCenter[2] = (int)( lensWidth * factors[2] ); //Center + colChWidth + shift );
	colChWidth = (int)( lensWidth * factors[3] );
	cchw = colChWidth/2;
	lensEndRange = (int)( lensWidth * factors[4] );
	printf( "lensNo=%d: lensStart=%d, lensWidth=%d,  chCenter=(%d, %d, %d),", lensNo, lensStart, lensWidth, chCenter[0], chCenter[1], chCenter[2] );
	printf( "colChWidth=%d, lensEndRange = %d \n", colChWidth, lensEndRange );

	// reconstruct the color information
	// read a strip of colChWidth pixels width and then stretch them over the whole lens width, then fill the gaps by interpolation
	for ( channel=0; channel<3; channel++ ) {
	  for ( y = 0; y < rgbFrame->height; y++ ) {
	    glValue = 0;
	    yy = y*yBinSize;
	    for ( ii=0; ii<=colChWidth; ii++ ) {
	      for ( j=0; j<yBinSize; j++ ) {
		yy += j;
		i = (int)((float)ii/colChWidth*(lensWidth-1));
		rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] += glScan->img[yy][lensStart+chCenter[channel]+ii-cchw];
	      }
	      rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)]/yBinSize;
	    }
	    // now interpolate to fill up the gaps by linear interpolation
	    i_left = 0;
	    for ( ii=1; ii<=colChWidth; ii++ ) {
	      i_right = (int)((float)ii/colChWidth*(lensWidth-1));
	      buf1 = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i_left)];
	      buf2 = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i_right)];
	      for ( i=i_left+1; i<i_right; i++ ) {
		rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] = ( (buf1*(i_right-i) +  buf2*(i -i_left)) /(i_right - i_left) );
	      }
	      i_left = i_right;
	    }
	    
	    // add in the black lines using lens end region!!!!!!!!!!!!!!!!!!!!!!!!!!!! ***CURRENTLY HARD CODED FIXED WIDTH VERSION***

	    //rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+0)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-1)] = glValue*5/6;
	    //rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+1)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-2)] = glValue*11/12;
	    //rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+2)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+lensWidth-3)] = glValue*39/40;

	    //rgbFrame->img[channel][y][rgbFrameWidth-(lensStart)] = rgbFrame->img[channel][y][rgbFrameWidth-(lensStart)]/2;
	    
	    for ( i=0; i<5; i++ ) {
	      rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] = (int)( rgbFrame->img[channel][y][rgbFrameWidth-(lensStart+i)] * lensEndWeights[i] );
	      rgbFrame->img[channel][y][rgbFrameWidth-(lensEnd-1-i)] = (int)( rgbFrame->img[channel][y][rgbFrameWidth-(lensEnd-1-i)] * lensEndWeights[i] );
	    }
...
int status = 0;
  ...
  if ( status == 0 ) {
    ...
    for ( i=0; (i<positions->length)&&(status==0); i++ ) {
      x = positions->list[i];  
      //printf( "line %d at x = %d\n", i, x );
      // extracted lines in green
      if ( x>=0 && x<=x_max ) {
        if ( profile->glSum[x] == 1  ) {
          for ( y=0; y<=y_max; y++ ) {
            rgbImage->img[0][y][x] = 3; //// Why /= 3 ?
            rgbImage->img[1][y][x] = 255;
            rgbImage->img[2][y][x] = 3;
          }
        }
	// extrapolated lines in magenta
