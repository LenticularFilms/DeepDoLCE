...
int main( int argc, char *argv[] ) {
  ...
  if ( set_greeter( &greetingText ) < 0 ) status=-1;
  char *copyrightText;
  if ( set_copyright( &copyrightText ) < 0 ) status = -1;
  char *helpText;
  if ( set_help( &helpText ) < 0 ) status = -1;
    
  // strings from required parameters
  ...
  float subPixelPrecisionRasterSpacing = 0;
  float lensWidth = 0;
  ...
  char horProfileRelaxRegRasterName[128] = "";
  ...
  //
  // ******* MOVE THIS INTO A FUNCTION CALLED 'call_parsing()' ****************
  //
  ...
  if ( argc < 6 ) {
    ...
    for ( argNo=1; argNo<argc-5 && status==0; argNo++ ) {
      ...
      else if ( strlen(argv[argNo])==5 && strncmp(argv[argNo],"-mode",5)==0 ) {
	rawMode = atoi( argv[++argNo] );
	vertBinning = atoi( argv[++argNo] );
	horResMode = atoi( argv[++argNo] );
	printf( "Mode summary: \n" );
	printf( "\trawMode = %d (1=deconvolute color values, 2=raw color values)\n", rawMode );
	printf( "\tvertBinning = %d\n", vertBinning );
	printf( "\thorResMode = %d\n", horResMode );
	if ( horResMode == 3 && rawMode == 1 ) {
	  printf( "ERROR: Invalid Mode: a=1 is incompatible with c=3! Try again using a=2.\n" );
	  status = -1;
	}
	if ( horResMode == 4 && rawMode == 1 ) {
	  printf( "ERROR: Invalid Mode: a=1 is incompatible with c=4! Try again using a=2.\n" );
	  status = -1;
	}
      }
        ...
        lockRasterSpacing = 1;
        argNo++;
        subPixelPrecisionRasterSpacing = atof( argv[argNo] );
        printf( "lockRasterSpacing is ON\n" );
        printf( "using subPixelPrecisionRasterSpacing = %f\n", subPixelPrecisionRasterSpacing );
      }
      else if ( strlen(argv[argNo])==12 && strncmp(argv[argNo],"-relaxRaster",12)==0 ) {
        relaxRaster = 1;
	argNo++;
	relaxDist = atoi(argv[argNo]);
        printf( "relaxRaster mode is ON\n" );
	printf( "using relaxDist = %d\n", relaxDist );
      }
      else {
        status = -1; 
      ...
      if ( argc-argNo < 5 ) {
        ...
        if ( strlen(startNoStr) == strlen(endNoStr) ) {
          ...
          if ( strlen(startNoStr)>1 && startNoStr[0]==0 ){
            ...
            sprintf( frameNoFormat, "%ld", strlen(startNoStr) );
            //printf( "frameNoFormat = %s\n", frameNoFormat );
          }
        }
      }
	...
	sprintf( frameNoStr, frameNoFormat, frameNo );
	  ...
	  sprintf( horProfileRelaxRegRasterName, "%s/%s/horizontalProfile_relaxedRegularizedRaster_%s%s.txt", inputDirName, outputDirName, inputBaseName, frameNoStr );
	...
	//// PART CONCERNING PERFORATION starts here
	...
	if ( status==0 ) {
	  ...
	  for ( i=0; i<4; i++ ) {
	    ...
	    if ( perfArea[i].memState != 1 ) {
	      status = -1;
	      printf( "ERROR: Cannot search for perforation : 'perfAreas.memState != 1'\n" );
	    }
	  }
	...
	/// end of part concerning perforation BUT somehow commenting it out doesn't change the rgb output!!!!!!!!!!!!
        ...
        if ( status == 0 ) { 
          ...
          if ( status==0 && horProfile.memState==1 ) {
            ...
            if ( status == 0 ) {
              ...
              if ( relaxRaster == 1 ) {
                status = copy_glProfile( &horProfile, &horProfileCpy ); // definitely better to keep the original glProfile instead of the smoothed one
                if ( status == 0 ) {
		...
		//status = -1;
                if ( lockRasterSpacing == 0 ) { ///// PROBABLY THIS SHOULD BE DONE ALWAYS EVEN WITH lockRasterSpacing == 1, it's more a kind of check that things are fine
                  ...
                  if ( status == 0 ) {
                    ...
                    if ( status == 0 ) {
                      ...
                      status = get_rasterSpacing( peakSpacingHist, &rasterSpacing ); /// HERE A FLOAT SHOULD COME OUT FOR "rasterSpacing" (should be called "lensWidth" since that's what it is)
                      if ( status == 0 ) {
		...
		////// this part causes a problem in case the option "-rasterSpacing" isn't used unless the a priori known value is added in the code, which is a bad choice anyways
		////////// change this into a criterion to discard the raster determined if it doesn't roughly match the input lensWidth, e.g. statistics of neighbor spacing, etc.
		if ( status == 0 ) {
		...
		/////////////////////////////////////////////////////////// HERE STARTed TO MODIFY..........................2017-12-04
                if ( status ==0 ) {
		  /* the call to the old raster regularization
                  printf( "Regularizing peaks...\n" );
                  //  rasterSize = nPeaks;
                  status = regularize_peakRaster( &horProfile, rasterSpacing, &rasterSize );
		  */

		  // call to a modified version of the regularization method
                  printf( "Regularizing peaks by modified method...\n" );
                  //  rasterSize = nPeaks;
		  lensWidth = subPixelPrecisionRasterSpacing;
		  printf( "lensWidth = %f\n", lensWidth );
                  status = regularize_peakRaster_mod( &horProfile, lensWidth, &rasterSize );
		  //
		  
		  /* the call to the new raster regularization
		  printf( "Generating profile for raster (=lens end positions)\n" );
		  status = generate_lensPosProfile( &horProfile, subPixelPrecisionRasterSpacing, &rasterSize ); // rastersize = number of lenses,       THIS IS THE NEW RASTER LOCALIZATION !!!!!!!!!!!!!!!!!!!
		  */
		      ...
		      // old method:
		      status = relax_regPeakRaster( &horProfileCpy, &horProfile, relaxDist );
		      // new method: (nothing ready yet...
		      //status = relax_regPeakRaster_mod( &horProfileCpy, &horProfile, relaxDist );
		      if ( troubleshoot == 1 ) {
			printf( "> storing profile containing relaxed regularized raster to '%s'\n", horProfileRelaxRegRasterName );
			status = write_glProfile( &horProfile, horProfileRelaxRegRasterName ); 
		      }
                    }
                    if ( status == 0 ) {
	...
	// }
	  ...
	  if ( horResMode == 3 || horResMode == 4 ) {
	    rgbFrameWidth = glScan.width; //( rasterPos.list[rasterPos.length-1] - rasterPos.list[0] + 1 );
	    rgbFrameHeight = glScan.height/vertBinning;
	    printf ( "UHR mode: rgbFrameWidth = %d, rgbFrameHeight = %d\n", rgbFrameWidth, rgbFrameHeight );
	    printf ( "          rasterPos.list[length-1] = %d, rasterPos.list[0] = %d\n", rasterPos.list[rasterPos.length-1], rasterPos.list[0] );
	    //status = -1;//////////////////HERE
	  }
	  else if ( horResMode == 2 ) {
            rgbFrameWidth = 2*(rasterSize-1)-1;
            rgbFrameHeight = glScan.height/vertBinning; //(int)((rgbFrameWidth)*1.0+0.0);    //*(glScan.height))/glScan.width;
          }
	  else if ( horResMode == 1 ) {
	    ...
	    printf( "ERROR: Unexpected horresMode=%d\n", horResMode );
	    status = -1;
          }
	    ...
	    if ( horResMode == 4 ) {
	      status = reconstructUHR_colorFrame( &glScan, &rasterPos, &rgbFrame );
	    }
	    else if ( horResMode == 3 ) {
	      status = reconstructFULL_colorFrame( &glScan, &rasterPos, &rgbFrame );
	    }
	    else if ( horResMode == 2 ) {
              status = reconstructInterpolate_colorFrame( &glScan, &rasterPos, &rgbFrame, &rawMode );
            }
	    else if ( horResMode == 1 ) {
	      status = reconstructRAW_colorFrame ( &glScan, &rasterPos, &rgbFrame, &rawMode );
	    }
            else {
          ...
          if ( status == 0  ) {
            ...
            for( channel=0; channel<3; channel++ ) {
              diff = rgbMaxVal[channel]-rgbMinVal[channel];
	      if ( diff > 0 ) {
		factors[channel] = 255.0/diff; // for proper white balance this has to be made in a different manner!!!!
	      }
	      else {
		factors[channel] = 1.0;
	      }
